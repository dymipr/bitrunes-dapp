...
import { UpdateRuneInfoModal } from "./UpdateRuneInfoModal";

export function DetailsSection({ details }: { details: RuneDetails }) {
	const [myAddress, setMyAddress] = useState<string | null>(null);
	const [modalOpen, setModalOpen] = useState(false);

	useEffect(() => {
		const address = localStorage.getItem("ordinalAddress");
		if (address) {
	...
	return (
		<>
								...
								<div className="relative space-x-2 flex">
									...
									</Button>
									{
										details.etching !== myAddress ?
											...
											:
											<Button
												className='w-min h-[1.6875rem] max-md:w-full max-md:h-[2rem] max-md:text-[0.90rem] disabled:bg-inherit'
												colorPallete='default' size='sm'
											>
												Update
											</Button>
									}
									<div className="absolute right-5">
										<MoreData address={details.etching} />
									</div>
			...
			<UpdateRuneInfoModal open={modalOpen} setOpen={setModalOpen} wallet={details.etching} />
		</>
	);
}
